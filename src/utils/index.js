...
export function getPoolLink(token0Address, token1Address = null, remove = false) {
  if (!token1Address) {
    return (
      `https://elephant.ac/#/` +
      (remove ? `remove` : `add`) +
      `/${token0Address === '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2' ? 'ETH' : token0Address}/${'ONE'}`
    )
  } else {
    return (
      `https://elephant.ac/#/` +
      (remove ? `remove` : `add`) +
      `/${token0Address === '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2' ? 'ONE' : token0Address}/${
        token1Address === '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2' ? 'ONE' : token1Address
      }`
    )
  }
}
...
export function getSwapLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `https://elephant.ac/#/swap?inputCurrency=${token0Address}`
  } else {
    return `https://elephant.ac/#/swap?inputCurrency=${
      token0Address === '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2' ? 'ONE' : token0Address
